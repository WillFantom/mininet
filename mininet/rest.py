...
if bottleImported:
    ...
    class REST():
        "Alternative REST class for missing bottle."

        def __init__( self, mininet, port=8080, *args, **kwargs ):
            """Bottle for Python 2 is no longer being maintained, 
            thus this class acts as a dummy for installs where python3-bottle 
            could not be found (no longer in apt from ubuntu groovy)"""
            self.mn = mininet
            self.port = port
            error( "Bottle Missing: API requires a Python3 install\n" )
